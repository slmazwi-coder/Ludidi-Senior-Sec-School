...
async function askGemini(userMessage: string): Promise<string> {
  try {
    const text = await generateChatResponse(userMessage, SYSTEM_PROMPT);
    if (!text) throw new Error('Empty response');
    return text;
  } catch (err) {
    console.error('[Chatbot] Gemini request failed:', err);
    return 'I\'m having trouble connecting right now. Please contact the school directly at +27 71 167 3161 or 200500449@ecschools.org.za.';
  }
}
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'bot',
      createdAt: Date.now(),
      text: "👋 Hello! Let me help you! Whether it's admissions, fees, results, activities or anything else about Ludidi SSS — just ask and I'll be happy to assist.",
    },
  ]);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: uid(),
          role: 'bot',
          text: 'Something went wrong. Please contact the school at +27 71 167 3161.',
          createdAt: Date.now(),
        },
      ]);
    } finally {
